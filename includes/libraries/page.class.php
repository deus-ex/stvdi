<?php
  ...
  **/
  ...
  class Page {
    ...
    *
    **/
    private function build( $pageStr ) {
      $customPage = $this->themeDirectory . $this->defaultTheme . $this->set_page( $pageStr );
    ...
    *
    **/
    private function set_page( $pageStr ) {
      return $pageStr . $this->extension;
    }
    ...
    *
    **/
    public function load( $pageStr = NULL ) {
      extract( $GLOBALS );

      if ( ! empty( $pageStr ) ) {
        include( $this->build( $pageStr ) );
      } else {
